...
% %     % delta_t_matrix=2
% %     % intrinsic_delay=0.
% %     % theta_0_1D_matrix=(linspace(0.1,3,30));
% %     % file_name_pc=['2021.2.8_with_int_delay_D_0=',num2str(D_0),'int_delay=',num2str(intrinsic_delay)];
% %     % 
% %     % delta_t_matrix=2
% %     % intrinsic_delay=0.1
% %     % theta_0_1D_matrix=(linspace(0.1,3,30));
...
% %     intrinsic_delay=0.1
% %     theta_0_1D_matrix=(linspace(0.1,3,30));
% %     file_name_pc=['2021.2.8_with_delta_t=10,int_delay_D_0=',num2str(D_0),'int_delay=',num2str(intrinsic_delay)];
% 
%         delta_t_matrix=5
%     intrinsic_delay=0.1
%     theta_0_1D_matrix=(linspace(0.1,3,30));
%     file_name_pc=['2021.2.9_with_delta_t=5,int_delay_D_0=',num2str(D_0),'int_delay=',num2str(intrinsic_delay)];
% 
%     
%     
%     file_name_vik=['2021.2.8_scan_vik_D_0=',num2str(D_0)];
...
%     clearvars -except D_0 file_name_pc file_name_vik
    ...
    for i=1:length(delta_t_matrix_vik)
        ...
        plot3(ones(length(AvN_plot))*delta_t_matrix_vik(i),AvN_plot,(1*rateNeff_plot(i,:)).*iF((-1 + AvN_plot)),'m-.')
        plot3(ones(length(AvN_plot))*delta_t_matrix_vik(i),AvN_plot,(2*rateNeff_plot(i,:)).*iF((-1 + AvN_plot)),'m-.')
    end
    sim=plot(1,1,'k-x')
    numeric=plot(1,1,'g-.')
    Kramer=plot(1,1,'r-.')
    ...
    legend([sim numeric Kramer numeric_eff Kramer_eff],{'Simulation', 'Numeric FPE, D_\theta=4D_0/(R^2\theta_0^2))', 'Kramers, D_\theta=4D_0/(R^2\theta_0^2))','Numeric FPE, D_\theta=2D_0/R^2','Kramers, D_\theta=2D_0/R^2'})
    v = [5 2 5];
    [caz,cel] = view(v);
    grid on
    xlabel('\delta t')
...
%         plot(AvN_plot,(2*rateN_plot(i,:)),'r--')
        ...
        plot(AvN_plot,(1*rateNeff_plot(i,:)).*iF((-1 + AvN_plot)),'m-.')
        plot(AvN_plot,(2*rateNeff_plot(i,:)).*iF((-1 + AvN_plot)),'m-.')
        set(gca,'YScale','log')
