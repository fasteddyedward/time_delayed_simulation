...
%     omega(k)=cross(R1',v(:,k))/norm(R1)^2;
    omega(k)=(R1(1)*v_y(k)-R1(2)*v_x(k))/norm(R1)^2;
    1;
end
% figure
% plot(omega./theta)
...
switch recalculate_T_eff
    case 'yes'
        ...
        % [fitresult,gof]=fit_Gaussian(Bins_theta2,Values_theta2,'yes');
        
        % a1*exp(-((x-b1)/c1)^2)
        mu_theta2=fitresult.b1;
        sigma_theta2=fitresult.c1/sqrt(2);
        %% 2021.1.12 hist(omega-\pm theta_+)
        
        [omega_stable,k_trans_omega,omega_plus,omega_minus,num_transitions_omega]=find_theta_plus(omega,theta_0);
        
        %         theta_plus=sqrt(10-sqrt(1/(42*theta_0)+120/theta_0-20));
        %         theta_minus=-sqrt(10-sqrt(1/(42*theta_0)+120/theta_0-20));
        
        omega2(1:length(theta))=omega-omega_stable;
...
% load(['nth_take=',num2str(nth_take),'.mat']);
figure(1)
...
%             theta_stable(k)=theta_minus;
        ...
        if sign_current==1
            theta_stable(k)=omega_plus;
        elseif sign_current==-1
            theta_stable(k)=omega_minus;
        end
