...
% v_0_matrix(1)=[];
    ...
    %     title('Data points')
    %     % hold off
        plot(v_0_interest,num_transitions_interest,'o')
        %% Fitting concerning non-constant R with curve tool
        figure(7);clf;
        R_interest=R_matrix(v_0_matrix>2*a/delta_t);
        V_R_ratio=v_0_interest./R_interest;
        plot(V_R_ratio,num_transitions_interest,'o')
        %          createFits(v_0_interest, num_transitions_interest, delta_t_interest, v_0_interest_head, num_transitions_interest_head, num_trans_interest, V_R_ratio)
        createFits(V_R_ratio, num_transitions_interest)
        
        %% Fitting concerning non-constant R with self
        figure(8); clf
        ...
        a=1000
        b=100
        f=@(x) a.*exp(-1.5*b.*(x*2-1).^2/(1*2));
        plot(V_R_ratio,f(V_R_ratio))
        %% Fitting concerning constant R with self
        figure(9); clf;
        plot(v_0_interest,num_transitions_interest,'o')
        hold on
        a=1000
        b=30
        f=@(x) a.*exp(-1.5*b.*(x*2/10-1).^2/(1*2));
        plot(v_0_interest,f(v_0_interest))
