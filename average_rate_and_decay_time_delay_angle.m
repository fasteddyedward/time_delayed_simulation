...
for iAv = 1:1:length(Av)
    ...
    for iruns = 1:runs
        ...
        Vim = (xV(2:end)-xV(1:end-1))/dt;
        
        Vavr = xV(ntau + 1:end) - xV(1:end-ntau);
        ...
        sigma2 = trapz(OmegaH,OmegaH.^2.*valH) - (trapz(OmegaH,OmegaH.*valH))^2;
        Dcalc = sigma2*dt/2;
        Teff = Dcalc*p1/2;
...
% subplot(1,2,2)
hold on
plot(AvN, AratesV,'DisplayName','simulation')
plot(AvN,1./(1*rateN).*iF((-1 + AvN)),'LineStyle','--','DisplayName','numerics')
plot(AvN,1./(1*rateA).*iF((-1 + AvN)),'LineStyle','-.','DisplayName','Kramers')
plot(AvN,1./(1*rateAeff).*iF((-1 + AvN)),'LineStyle',':','DisplayName','KramersEff')
plot(AvN,1./(2*rateN).*iF((-1 + AvN)),'LineStyle','--','DisplayName','numerics2')
plot(AvN,1./(2*rateA).*iF((-1 + AvN)),'LineStyle','-.','DisplayName','Kramers2')
plot(AvN,1./(2*rateAeff).*iF((-1 + AvN)),'LineStyle',':','DisplayName','KramersEff2')
